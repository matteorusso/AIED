...
def age_convert(y):
    if y == None or math.isnan(y):
        return 0
    a = 2018 - int(y) # leave it as 2018 because people were taking courses back then
    if a> 70 or a< 10:
        a = 0
    return a
...
# extract user gender
user_gender = dict(zip(user_profile.index, user_profile.gender))#user_gender = user_profile['gender'].to_dict()
...
user_cluster_id = pkl.load(open('cluster/user_dict','rb'), encoding='latin1')
...
def category_convert(cc):
    if isinstance(cc, str):
        for i, c in zip(range(len(en_categories)), en_categories):
            if cc == c:
                return i+1
    else:
        return 0
category_dict = dict(zip(courseinfo.course_id, courseinfo.category)) #category_dict = courseinfo['category'].to_dict()
...
# Unscaled data
all_feat.loc[train_feat.index].to_csv('u_train_feat.csv')
all_feat.loc[test_feat.index].to_csv('u_test_feat.csv')
